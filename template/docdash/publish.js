...
function addAttribs(f) {
  ...
  if (attribsString && attribsString.length) {
    f.attribs = util.format(
      '<span class="type-signature type-signature-' +
        attribsString.replace(/\(/g, '').replace(/\)/g, '').trim() +
        '">%s</span>',
      attribsString
    )
  } else {
...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
  ...
  if (items && items.length) {
    ...
    var level =
      typeof docdash.navLevel === 'number' && docdash.navLevel >= 0 ? docdash.navLevel : Infinity
    ...
    if (itemsNav !== '') {
      if (docdash.collapse === 'top') {
        nav +=
          '<h3 class="collapsed_header">' +
          itemHeading +
          '</h3><ul class="collapse_top">' +
          itemsNav +
          '</ul>'
      } else {
