...
const BeatTitleCell = ({
  ...
  useEffect(() => {
    if (
      ...
      // @ts-ignore
      document.activeElement !== titleInputRef
    ) {
      // @ts-ignore
  ...
  const handleDrop = useCallback(
    (e) => {
      ...
      const droppedBeat = helpers.json.safeParseJSON(json)
...
const makeMapState = () => {
  ...
  return function mapStateToProps(state, ownProps) {
    return {
      ...
      beat: uniqueBeatsSelector(
        state,
        // @ts-ignore
        ownProps.beatId
      ),
      hierarchyLevel: selectors.hierarchyLevelSelector(
        state,
        // @ts-ignore
        ownProps.beatId
      ),
      beatTitle: uniqueBeatTitleSelector(
        state,
        // @ts-ignore
        ownProps.beatId
      ),
      ...
      atMaximumDepth: selectors.atMaximumHierarchyDepthSelector(
        state,
        // @ts-ignore
        ownProps.beatId
      ),
      ...
      editing: selectors.editingGivenBeatsTitleSelector(
        state,
        // @ts-ignore
        ownProps.beatId
      ),
      selection: selectors.timelineBeatSelectionSelector(
        state,
        // @ts-ignore
        ownProps.beatId
      ),
