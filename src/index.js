...
import { actions, migrateIfNeeded, emptyFile, ARRAY_KEYS } from 'pltr/v2'
...
export const listenToFile = (store, userId, fileId, clientId) => {
  const identity = (x) => x
  return database()
    .collection('file')
    .doc(fileId)
...
export const initialFetch = (store, userId, fileId, clientId, version) => {
  return Promise.all([
...
export const saveCustomTemplate = (userId, template) => {
  return database().collection(`templates/${userId}`).add(template)
}
