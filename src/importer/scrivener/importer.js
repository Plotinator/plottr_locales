...
import { last, isObject, cloneDeep, omit, uniqWith, isEqual, range } from 'lodash'
...
function importScrivenerFile(
  ...
  const files = new Map()
  const thunkFile = (withData) => (filePath, containingFolderPathComponents, extension) => () => {
    if (files.has(filePath)) {
      return Promise.resolve(files.get(filePath))
    } else {
      return readFile(filePath).then((contents) => {
        const value = withData(contents, filePath, containingFolderPathComponents, extension)
    ...
    if (!root) {
      ...
      return Promise.all([
        ...
        allTxtAndRTFFiles(
          rootPath,
          listFilesRecursively,
          thunkFile(transformRTFOrIdentityOnText(convertRTFToSlate))
        ),
      ])
...
function transformRTFOrIdentityOnText(convertRTFToSlate) {
  return (content, _filePath, _containingFolderPathComponents, extension) => {
    if (extension === '.rtf') {
...
export function extractPlotlines(scrivenerStructure, bookId) {
  function accumulateLines(currentLines, next) {
    ...
    if (Array.isArray(next.data)) {
      return Promise.all([sequenceThunks(next.data.map(getDataThunk)), withChildren]).then(
        ([contents, lines]) => {
        ...
        return () => {
          ...
          return iter(id, children || []).then((childrenCards) => {
            if (kind === 'Folder') {
              ...
              return sequenceThunks(data.map(getDataThunk)).then((contents) => {
                const attributes = findAndExtractAttributes(contents, CARD_BASE_ATTRIBUTES)
    ...
    scrivenerStructure.notes.map((note) => () => {
      ...
      if (Array.isArray(data)) {
        ...
        return Promise.resolve(
          createNote(id, title, content, tags, characters, places, category, attributes)
        )
      }
    })
  )
  ...
  return sequenceThunks(
    scrivenerStructure.characters.map((character) => () => {
      ...
      if (Array.isArray(data)) {
        ...
        const attributes = {}
        return Promise.resolve(
          createCharacter(id, title, description, notes, tags, category, attributes)
        )
      }
    })
  )
  ...
  return sequenceThunks(
    scrivenerStructure.places.map((place) => () => {
      ...
      if (Array.isArray(data)) {
        ...
        const attributes = {}
        return Promise.resolve(
          createPlace(id, title, description, notes, tags, category, attributes)
        )
      }
    })
  )
 ...
 */
function allTxtAndRTFFiles(rootPath, listFilesRecursively, thunkFile) {
  return listFilesRecursively(rootPath)
    .then((files) => {
      return files.reduce(
        (acc, { fullPath, containingFolderPathComponents, fileName, extension }) => {
          if (extension === '.rtf' || extension === '.txt') {
            const containingFolderName = last(containingFolderPathComponents)
            if (containingFolderName === OLD_SCRIVENER_CONTAINING_FOLDER_NAME) {
              const id = filesEntityId(fileName)
              return [
                ...acc,
                [
                  id,
                  {
                    fullPath,
                    data: thunkFile(fullPath, containingFolderPathComponents, extension),
                  },
                ],
              ]
            } else {
