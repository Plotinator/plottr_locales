            ...
            return readFile(filePath)
              ...
              .then((subscriptions) => {
                return sequencePromises(
                  subscriptions.map(({ userId, subscription }, index) => {
                    return () => {
                      console.log(`> ${userId} (${index + 1}/${subscriptions.length})`)
                      const { plottrSubscription, proSubscription } = subscription
                      const plottrLicense = {
                        plottrLicense: plottrSubscription,
                        proLicense: proSubscription,
                      }
                      console.log('Writing', plottrLicense)
                      ...
                      const addLicenseLogRecord = admin
                        .firestore()
                        .collection(
                          `licenseChangesFromImports/import-licenses-from-edd-dump/${userId}`
                        )
                        .add({ plottrLicense, date: now })
