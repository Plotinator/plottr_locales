  ...
  return theWorld.settings.listenToExportConfigSettingsChanges(store, (error, newValue) => {
    if (error) {
      if (error.code === 'ECONNABORTED') {
        theWorld.logger.warn(`Failed to receive export config (ECONNABORTED; likely a timeout)`)
      } else {
        theWorld.logger.warn(`Failed to receive export config`, error)
      }
    } else {
...
export const publishAppSettingsChangesToStore = (theWorld) => (store) => {
  ...
  return theWorld.settings.listenToAppSettingsChanges(store, (error, newValue) => {
    if (error) {
      if (error.code === 'ECONNABORTED') {
        theWorld.logger.warn(`Failed to receive app settings (ECONNABORTED likely a timeout)`)
      } else {
        theWorld.logger.warn(`Failed to receive app settings`, error)
      }
    } else {
      store.dispatch(action(newValue))
    }
