...
export const publishTemplatesChangesToStore = (theWorld) => (store) => {
  ...
  return theWorld.templates.listenToTemplatesChanges(store, (error, newValue) => {
    if (error) {
      theWorld.logger.error(`Failed to receive templates`, error)
    } else {
...
export const publishCustomTemplatesChangesToStore = (theWorld) => (store) => {
  ...
  return theWorld.templates.listenToCustomTemplatesChanges(store, (error, newValue) => {
    if (error) {
      theWorld.logger.error(`Failed to receive custom templates`, error)
    } else {
      store.dispatch(action(newValue))
    }
  })
...
export const publishTemplateManifestChangesToRedux = (theWorld) => (store) => {
  ...
  return theWorld.templates.listenToTemplateManifestChanges(store, (error, newValue) => {
    if (error) {
      theWorld.logger.error(`Failed to receive template manifest`, error)
    } else {
      store.dispatch(action(newValue))
    }
  })
