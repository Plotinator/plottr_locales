...
export const theWorldAccordingToPlottr = {
  ...
  license: PropTypes.shape({
    persistUserId: PropTypes.func.isRequired,
    persistEmailAddress: PropTypes.func.isRequired,
    persistLicenseMode: PropTypes.func.isRequired,
...
export const plottrWorldAPI = (theWorld) => {
  ...
  const {
    ...
    settings: { currentAppSettings, currentExportConfigSettings },
  } = current(theWorld)

  const api = {
    logger,
    publishChangesToStore: publishChangesToStore(theWorld, logger),
    license: {
      currentLicense,
      currentTrial,
    },
