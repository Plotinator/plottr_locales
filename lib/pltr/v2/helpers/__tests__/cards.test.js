        ...
        describe('when the name is valid', () => {
          it('should produce the path to a base attribute', () => {
            expect(
              cardFocusPath('3', {
                // @ts-ignore
                baseAttributeName: 'title',
              })
            ).toEqual(['card', 3, 'title'])
          })
        })
      })
          ...
          describe('but the id or attribute name are not strings', () => {
            it('should produce the unknown path', () => {
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: 4, attributeName: 'test' },
                })
              ).toEqual(['unknown'])
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: null, attributeName: 'test' },
                })
              ).toEqual(['unknown'])
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: undefined, attributeName: 'test' },
                })
              ).toEqual(['unknown'])
              ...
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: 'c3', attributeName: 4 },
                })
              ).toEqual(['unknown'])
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: 'c3', attributeName: null },
                })
              ).toEqual(['unknown'])
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: 'c3', attributeName: undefined },
                })
              ).toEqual(['unknown'])
            })
          })
          describe('and the id and attribute names are strings', () => {
            it('should produce a card focus path', () => {
              expect(
                cardFocusPath('3', {
                  // @ts-ignore
                  template: { id: 'c3', attributeName: 'test' },
                })
              ).toEqual(['card', 3, 'template', 'c3', 'test'])
            })
          })
        })
      })
