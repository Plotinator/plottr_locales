 ...
 * version
 **/
module.exports = program.version(pkg.version)
...
program
  ...
  .option(
    '-l, --locale [locale]',
    'BCP 47 language tags specifying the source default locale [en]',
    'en'
  )
  .option(
    '-t, --translations [path]',
  ...
  .option(
    '-f, --filename [filename]',
    'filename to use when reading from stdin - this will be used in errors [stdin]',
    'stdin'
  )
  .option(
    '-s, --style [style]',
    ...
    if (options.extends === 'default') {
      ...
      if (options.customrules && existsSync(options.customrules)) {
        ...
        if (options.customrules) {
          errors.push(
            'A valid file needs to be provided for "customrules". "' +
              options.customrules +
              '" is not a valid file.'
          )
        } else {
          errors.push(
            'When "customrules" is specified for "extends", a valid path has to be provided for "customrules".'
          )
        }
        options.customrules = null
      }
      ...
      errors.push(
        'A valid extends value [default, recommended, customrules] needs to be provided. "' +
          options.extends +
          '" is not valid.'
      )
    }
...
program
  ...
  .option(
    '-f, --filename [filename]',
    'filename to use when reading from stdin - this will be used in errors',
    'stdin'
  )
...
program
  ...
  .option(
    '-e, --missing-translation [behavior]',
    'behavior when --translations is specified, but a translated pattern is missing (error | warning | ignore) [error]',
    'error'
  )
  .option(
    '-m, --missing-replacement [pattern]',
    'pattern to inline when a translated pattern is missing, defaults to the source pattern'
  )
  ...
  .option(
    '-f, --filename [filename]',
    'filename to use when reading from stdin - this will be used in source-maps, errors etc [stdin]',
    'stdin'
  )
