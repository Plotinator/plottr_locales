...
describe('format-message transform -i', function () {
  describe('stdin', function () {
    ...
    it('handles dotted placeholders for non-literal params', function () {
      ...
      expect(stdout.trim()).to.equal(
        '"hello " + ("a.b.c" in params ? params["a.b.c"] : params.a.b.c);'
      )
    })
    ...
    it('handles custom placeholders', function () {
      ...
      expect(stdout.trim()).to.contain(
        '"hello " + formatMessage.custom(["a", "b", "c"], "en", params.a, params);'
      )
    })
  ...
  describe('translations', function () {
    it('uses -t translations', function () {
      ...
      var cmd =
        'format-message transform -i -g underscored_crc32' +
  ...
  describe('out-dir', function () {
    ...
    var ofiles = readdirSync('packages/format-message/__tests__', 'utf8').filter(function (file) {
    ...
    it('uses -s source-maps', function () {
      ...
      var fileContent = readFileSync(dirname + '/index.spec.js', 'utf8').split(
        '//# sourceMappingURL='
      )
  ...
  describe('translate="yes"', function () {
    ...
    it('handles number, date, and time helpers', function () {
      var input =
        'import { number, date, time } from "format-message"\n' +
    ...
    it('handles select helpers', function () {
      var input =
        'var sel = require("format-message").select\n' +
    ...
    it('handles plural & selectordinal helpers', function () {
      var input =
        'var { plural, selectordinal: o } = require("format-message")\n' +
  ...
  describe('translate="yes"', function () {
    ...
    it('handles style names with spaces', function () {
      ...
      expect(stdout).to.contain(
        'id: "Caught { count, number } on { d, date, \'MMM d\' } at { t, time }"'
      )
      expect(stdout).to.contain(
        'default: "Caught { count, number } on { d, date, \'MMM d\' } at { t, time }"'
      )
    ...
    it('handles number, date, and time helpers', function () {
      ...
      expect(stderr).to.equal('')
      expect(stdout).to.contain(
        'id: "Caught { count, number } on { d, date, short } at { t, time }"'
      )
      expect(stdout).to.contain(
        'default: "Caught { count, number } on { d, date, short } at { t, time }"'
      )
      expect(stdout).to.contain('count: count')
      expect(stdout).to.contain('d: d')
      expect(stdout).to.contain('t: t')
