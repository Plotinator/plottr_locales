...
import { actions, emptyFile } from 'pltr/v2'
...
export const newFile = (userId, fileName, fullState) => {
      ...
      database()
        ...
        .then((permissionDocRef) => {
          ...
          return Promise.all(requests).then((results) => {
            return listen(userId, fileRef.id).then((listening) => {
              return fetchFiles(userId)
            })
          })
        })
    })
