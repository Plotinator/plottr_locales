...
import { updateAllKeys } from './middlewares/firebase-sync'
...
const listenToBeats = (userId, fileId, clientId, version) => {
  ...
  return database()
    .collection('beats')
    .doc(fileId)
    .onSnapshot(onSnapshot(fileId, 'beats', transform, true, clientId))
}
...
export const listen = (userId, fileId, clientId, fileVersion) => {
  const unsubscribeFunctions = [
    listenToFile(userId, fileId, clientId),
    listenToBeats(userId, fileId, clientId, fileVersion),
  ...
  return Promise.all([
    ...
    return new Promise((resolve, reject) => {
      migrateIfNeeded(appVersion(), json, json.file.fileName, null, (error, migrated, data) => {
        ...
        updateAllKeys(fileId, clientId, data).then((results) => {
          store.dispatch(
            actions.ui.loadFile(
              data.file.fileName,
              false,
...
const getSingleDocument = (documentRef) => {
  ...
  if (documents.length) {
    return { document: documents[0], documentRef }
  }
  return null
}

const startOfSessionBackup = (userId, file, startOfToday) => {
  return database()
...
const currentBackup = (userId, file, startOfToday) => {
  return database()
    .collection(`backup/${userId}/files`)
    .where('fileId', '==', file.fileId)
    .where('backupTime', '==', startOfToday)
...
export const saveBackup = (userId, file) => {
  ...
  return startOfSessionBackup(userId, file, startOfToday).then((startOfSession) => {
    // Is there a backup for the start of today?
    if (startOfSession) {
      // Is there a non-start-of-session backup?
      return currentBackup(userId, file, startOfToday).then((result) => {
        if (result) {
