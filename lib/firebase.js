...
const onSnapshot =
  (fileId, path, withData, patching, loadFunctionKey = 'load') =>
  (documentRef) => {
    const data = documentRef.data()
    delete data.fileId
    store.dispatch(patchActions(path)[loadFunctionKey](patching, withData(data)))
  }
...
export const fetchFile = (userId, fileId) => {
  ...
  database()
    .collection('file')
    .doc(fileId)
    .onSnapshot(onSnapshot(fileId, 'file', identity, true, 'patchFile'))
}
