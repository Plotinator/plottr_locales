...
import { actions, initialState } from 'pltr/v2'
...
export const newFile = (userId, fileName) => {
  const newFile = {
    ...initialState.file,
    fileName,
  }
  return database()
    ...
    .then((fileRef) => {
      database()
        .collection(`authorisation/${userId}/granted/${fileRef.id}`)
        .set({
          permission: 'owner',
        })
        .then((permissionDocRef) => {
          const requests = [
            database().collection('beats').document(fileRef.id).set(initialState.beats),
            database().collection('cards').document(fileRef.id).set(initialState.cards),
            database().collection('series').document(fileRef.id).set(initialState.series),
            database().collection('books').document(fileRef.id).set(initialState.books),
            database().collection('categories').document(fileRef.id).set(initialState.categories),
            database().collection('characters').document(fileRef.id).set(initialState.characters),
            database()
              .collection('customAttributes')
              .document(fileRef.id)
              .set(initialState.customAttributes),
            database().collection('lines').document(fileRef.id).set(initialState.lines),
            database().collection('notes').document(fileRef.id).set(initialState.notes),
            database().collection('places').document(fileRef.id).set(initialState.places),
            database().collection('tags').document(fileRef.id).set(initialState.tags),
            database()
              .collection('hierarchyLevels')
              .document(fileRef.id)
              .set(initialState.hierarchyLevels),
            database().collection('images').document(fileRef.id).set(initialState.images),
          ]
