...
import {
  ...
  saveCustomTemplate as saveCustomTemplateToFirestore,
    ...
    .then((response) => {
      if (response.status == 200) {
        ...
        if (force || fetchedManifestIsNewer(manifest)) {
          ...
          logger.info('new templates found', manifest.version)
          return fetchTemplates(force, manifest)
        } else {
          logger.info('No new template manifest', manifest.version)
          return fetchTemplates(force, manifest)
        }
      } else {
...
export const saveCustomTemplateToStorage = (template) => {
  window.localStorage.setItem(customTemplateKey(template.id), JSON.stringify(template))
}

export const saveCustomTemplate = (userId, template) => {
  saveCustomTemplateToFirestore(userId, template)
    .then(() => {
      saveCustomTemplateToStorage(template)
    })
    .catch((error) => {
      logger.error(`Error saving new template for user with id ${userId}`, error)
    })
}
...
export const listenToCustomTemplates = (userId) => {
  return listenToCustomTemplatesFromFirestore(
    userId,
    (customTemplates) => {
      return saveCustomTemplatesToStorage(customTemplates)
    },
    (error) => {
      logger.error(`Error fetching templates for user with id: ${userId}`, error)
    }
  )
}
