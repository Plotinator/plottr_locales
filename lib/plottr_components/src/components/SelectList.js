...
const SelectList = ({
  ...
  const renderSelectedTags = () => {
    ...
    return selectedItems.map((tId) => {
      const tag = allItems.find((item) => item.id == tId)
      if (!tag) return null
      return (
  ...
  const renderUnSelected = () => {
    ...
    if (categories) {
      ...
      listItems = available.map((category) => {
        ...
        return [
          ...
          ...categoryValues.map((id) => {
            ...
            if (listType === 'Tags') {
              colorSpan = <span className="colored" style={{ backgroundColor: item.color }}></span>
            }
            const fallbackName =
      ...
      listItems = itemsToList.map((i) => {
        ...
        const fallbackName = listType && endsWith(listType, 's') ? listType.slice(0, -1) : listType
        return (
    ...
    return (
      <Popover id="list-popover" title={title} noMaxWidth>
        ...
        <ul
          // @ts-ignore
          ref={selectListRef}
          className="select-list__item-select-list"
        >
          {listItems}
        </ul>
      </Popover>
