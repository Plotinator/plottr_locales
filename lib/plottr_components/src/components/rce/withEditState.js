...
const pathIsAfter = (thisPath, thatPath) => {
  ...
  for (let i = 0; i < maxDepth; ++i) {
    ...
    if (!thatNode || !thisNode) return false
...
const editsConflict = (editsAfter, operationsToApply) => {
  ...
  const furthestNonSplitOperationEdit = maxBy(
    operationsToApply.filter(({ operation: { type } }) => type !== 'split_node'),
    (operation) => {
      return pathToNumber(operation.path)
    }
  )
  ...
  const weSplitBeforeIncomingOperation =
    ...
    pathIsAfter(earliestPathOfEditAfterThatSplits, furthestNonSplitOperationEdit.path)
