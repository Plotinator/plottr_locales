...
const newOperationHistory = () => {
  return new PriorityQueue((thisOperation, thatOperation) => {
    return thisOperation.created < thatOperation.created
  })
}

const enqueueOperation = (queue, value, selection, operation) => {
  queue.add({ created: operation.created, value, selection, operation })
}

const findEditsAfter = (queue, operation) => {
  const editsAfter = []
...
export const withEditState = (
  ...
  const [state, setState] = useState(INITIALISED)
  ...
  useEffect(() => {
    if (state !== CONTENT_EDITED) return

    if (publishOperations && fileId && editorId) {
      const operationsToPublish = []
      editHistory.current.forEach((operation) => {
        if (operation.operation.editNumber > lastPublished.current) {
          operationsToPublish.push(operation.operation)
        }
      })
      const lastEditNumber = operationsToPublish.reduce((maxEditNumber, operation) => {
        if (maxEditNumber > operation.editNumber) return maxEditNumber
        else return operation.editNumber
      }, operationsToPublish.length && operationsToPublish[0].editNumber)

      publishOperations(fileId, editorId, key.current, operationsToPublish)
      lastPublished.current = lastEditNumber
    }
  }, [state])

  useEffect(() => {
    if (state !== RECEIVED_OPERATIONS) return
  ...
  useEffect(() => {
    if (state !== UPDATED_EDIT_TIME_STAMPS) return
    ...
    if (!isEqual(selection, editor.selection)) {
      ...
      if (
        ...
        if (value !== newValue) {
          ...
          onValueChanged(newValue, nextSelection)
        } else {
          setEditorState(value, nextSelection)
          onValueChanged(null, nextSelection)
        }
      }
    } else if (value !== newValue) {
      setEditorState(newValue, editor.selection)
  ...
  const handleNewInitialValue = () => {
    ...
    if (!undoId) {
      ...
      if (!selection) {
        nextValue = initialSelection || editor.selection
      }
    }
    try {
    ...
    setEditorState(nextValue, nextSelection)
    setState(UPDATED_FROM_INITIAL_VALUE)
  }
  ...
  const handleOtherEditorChange = (latestEditsPerEditor) => {
    ...
    setState(UPDATED_EDIT_TIME_STAMPS)
  }
  ...
  }, [initialValue, undoId, initialSelection])
