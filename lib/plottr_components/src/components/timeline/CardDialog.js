    ...
    <PlottrModal
      ...
      style={{
        content: {
          ...modalStyles.content,
          outline:
            cardMetaData.color && useCardColorAsOutline
              ? `1px solid ${cardMetaData.color}`
              : 'none',
        },
      }}
    >
...
CardDialog.propTypes = {
  ...
  useCardColorAsOutline: PropTypes.bool,
}
...
const mapStateToProps = (state, ownProps) => {
  return {
    cardMetaData: selectors.cardMetaDataSelector(
      state,
      // @ts-ignore
      ownProps.cardId
    ),
    ...
    useCardColorAsOutline: selectors.useCardColorAsOutlineSelector(state),
  }
}
