...
const UnconnectedBeatRow = ({
  ...
  const handleDragStartHere = (_event) => {
    handleDragStart(id)
  }
          ...
          {hierarchyLevels.map((level, index) => {
            return (
              <MenuItem key={index} onSelect={() => setHierarchyLevel(id, level)}>
                <div className="restructure-modal__hierarchy-level-selector">{t(level.name)}</div>
              </MenuItem>
            )
          })}
        </DropdownButton>
...
const mapBeatRowStateToProps = (state, ownProps) => ({
  title: uniqueBeatTitleSelector(
    state,
    // @ts-ignore
    ownProps.id
  ),
...
const BeatRow = connect(mapBeatRowStateToProps)(UnconnectedBeatRow)
...
const mapStateToProps = (state) => {
  return {
    beatHierarchyLevels: selectors.sortedBeatsHierachyLevels(state),
